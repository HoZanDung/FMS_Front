<template>
  <div class="panel">
    <panel-title :title="$route.meta.title">
      ...
      <router-link :to="{name: 'storageMainAdd'}" tag="span">
      ...
      <el-table
        ...
        <el-table-column
          ...
          <template scope="props">
            <router-link :to="{name: 'storageMainModify', params: {
                id: props.row.id,
                speciesAmount:props.row.speciesAmount,
                storageAmount:props.row.storageAmount,
                storageAllPrice:props.row.storageAllPrice,
                storageDate:props.row.storageDate,
                storageStatus:props.row.storageStatus,
                transactor:props.row.transactor,
                supplierNo:props.row.supplierNo}}" tag="span">
...
<script type="text/javascript">
  ...
  export default{
    ...
    methods: {
      ...
      get_table_data(){
        this.load_data = true
        this.$fetch.api_storageMain.list({
      ...
      delete_data(item){
        this.$confirm('此操作将删除该数据, 是否继续?', '提示', {
          ...
          .then(() => {
            let uid = item.id
            this.load_data = false
            this.$fetch.api_storageMain.deleteById(uid)
              .then(() => {
                this.get_table_data()
      ...
      recovery_data(item){
        this.$confirm('此操作将恢复该数据, 是否继续?', '提示', {
          ...
          .then(() => {
            let uid = item.id
            this.load_data = false
            this.$fetch.api_storageMain.recoveryById(uid)
              .then(() => {
                this.get_table_data()
